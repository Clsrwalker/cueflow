...
Cloud resources use HTTPS/WSS endpoints, S3 block public access, S3 managed encryption, SQS managed encryption, DynamoDB managed persistence, and least-privilege grants from resources to functions. Credentials are not committed. Deployment credentials are expected through GitHub OIDC or local AWS profile configuration.
